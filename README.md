...
Prerequisites
    ...
    Deno: Make sure you have Deno installed. If not, follow the instructions on deno.land.
    Git: For cloning the repository.
    OpenAI API Key: Obtain one from your OpenAI dashboard.
...
1. Prepare for Deployment
    ...
    git branch -M main
    git remote add origin https://github.com/diwakar-bhadriraju/VoiceBot.git
...
4. Deploy!
    ...
    Subsequent git pushes to your linked branch (e.g., main) will also trigger automatic deployments.

You'll get a unique URL for your deployed AI Chatbot!
🎙️ How to Use

    Open the Application: Navigate to http://localhost:8000 (locally) or your Deno Deploy URL (deployed).
    Microphone Permission: Your browser will likely ask for microphone permission. Grant it.
    Choose Voice: Select your preferred voice from the "Choose Voice" dropdown. Natural-sounding voices usually come from "Google", "Microsoft", or macOS system voices.
    Start Conversation: Click the "Start Conversation" button. The button will change to "Stop Listening," and the UI will indicate "Listening...".
    ...
    AI Response: The AI will process your input, display its response in the chat log, and then speak it aloud.
    Continue Conversation: Click "Start Conversation" again to speak your next message. The AI will remember the previous context.
    Clear Conversation: Click the "Clear Conversation" button to start a fresh dialogue, wiping the chat history.
    Dark Mode: Click the 🌙 / ☀️ icon in the top right corner to toggle the theme.
